...
generated by `predict_fn(state)` at a chosen POI value (`poi_asimov`). It
...
1. **`predict_fn`** (standard): Passed to `AsymptoticCalculator` or directly to
   the test statistic. The Asimov dataset is generated at `poi_asimov`.
   ...
   calc = AsymptoticCalculator(
       ...
       poi_key="mu",
       predict_fn=predict,
       poi_asimov=0.0,  # background-only Asimov
   )
   ```
...
- **`poi_asimov=0.0`** (default): Background-only hypothesis. Use for exclusion
  tests and upper limits.
- **`poi_asimov=1.0`**: Signal+background hypothesis. Use for discovery tests.

A scalar sets every tested POI to that value. In a multi-POI model, pass a
mapping to set each POI separately, e.g. `poi_asimov={"mu_a": 0.0, "mu_b": 1.0}`.
