...
# This matches evermore's internal structure for optimization
evm_dynamic, evm_static = evm.tree.partition(evm_params)

# Extract pure values from dynamic params as initial pytree
...
def evm_nll_fn(params_pytree):
    """NLL function that wraps evermore's loss.

    This demonstrates using evermore models with everwillow inference.
    The function reconstructs evermore's Params structure and calls
    the evermore loss function.

    Args:
        params_pytree: Dict of parameter values

    Returns:
        Loss value (note: evermore loss differs from pyhs3 NLL by a constant)
    """
...
# Print header
rich.print(f"{'Parameter':<10} {'pyhs3+evw':<15} {'evm+evw':<15} {'evm+native':<15} "
...
for param_name in ["mu", "norm1", "norm2", "shape1"]:
    ...
    evm_evw_val = result_evm.params[param_name]
    ...
    rich.print(f"{param_name:<10} {pyhs3_val:<15.6f} {evm_evw_val:<15.6f} {evm_native_val:<15.6f} "
