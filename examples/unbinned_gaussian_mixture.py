...
def pyhs3_nll(params):
    ...
    prior_nll = -jnp.log(
        jnp.exp(-((params["b"] - 1.0) ** 2) / (2 * 0.3**2))
        / (0.3 * jnp.sqrt(2 * jnp.pi))
    )
    nll += prior_nll
...
evm_params = {
    ...
    "b": evm.Parameter(
        value=1.0, name="b", prior=evm.pdf.Normal(mean=1.0, width=0.3)
    ),  # With prior
    "gauss1": {
...
dynamic, static = evm.tree.partition(
    evm_params, is_leaf=lambda x: isinstance(x, evm.Parameter)
)
...
def pytree_to_evm_params(pytree):
    ...
    return {
        ...
        "b": evm.Parameter(
            value=pytree["b"], name="b", prior=evm.pdf.Normal(mean=1.0, width=0.3)
        ),
        "gauss1": {
...
# Print header
rich.print(
    f"{'Parameter':<10} {'pyhs3+ew':<15} {'evm+native':<15} {'evm+ew':<15} "
...
comparisons = [
    ...
    (
        "mu1",
        "gauss1/mu",
        result_pyhs3.params["mu1"],
        evm_bestfit["gauss1"]["mu"].value,
        result_evm.params["mu1"],
    ),
    (
        "sigma1",
        "gauss1/sigma",
        result_pyhs3.params["sigma1"],
        evm_bestfit["gauss1"]["sigma"].value,
        result_evm.params["sigma1"],
    ),
    (
        "mu2",
        "gauss2/mu",
        result_pyhs3.params["mu2"],
        evm_bestfit["gauss2"]["mu"].value,
        result_evm.params["mu2"],
    ),
    (
        "sigma2",
        "gauss2/sigma",
        result_pyhs3.params["sigma2"],
        evm_bestfit["gauss2"]["sigma"].value,
        result_evm.params["sigma2"],
    ),
]
...
for param_name, _, pyhs3_val, evm_native_val, evm_ew_val in comparisons:
    ...
    rich.print(
        f"{param_name:<10} {pyhs3_val:<15.6f} {evm_native_val:<15.6f} {evm_ew_val:<15.6f} "
