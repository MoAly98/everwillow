...
class TestHypoTestResult:
    ...
    def test_with_all_fields(self):
        ...
        assert float(result.cl_s) == pytest.approx(0.02 / 0.85)
        assert result.test_stat_result is ts_result

    def test_none_pvalues(self):
        """pnull, palt, cl_s can all be None."""
        ...
        assert result.cl_s is None

    def test_expected_bands_defaults_none(self):
        """expected_bands defaults to None."""
        ts_result = TSResult(value=jnp.array(1.0), test=jnp.array(1.0))
        result = HypoTestResult(
            q_obs=jnp.array(1.0),
            pnull=jnp.array(0.05),
            palt=jnp.array(0.5),
            cl_s=jnp.array(0.1),
            test_stat_result=ts_result,
        )
    ...
    def test_expected_bands_stored(self):
        """expected_bands is stored when provided."""
        ts_result = TSResult(value=jnp.array(1.0), test=jnp.array(1.0))
        bands = _make_bands(
            [3.167e-5, 0.00135, 0.02275, 0.15866, 0.5],
            [0.02275, 0.15866, 0.5, 0.84134, 0.97725],
        )
        result = HypoTestResult(
            q_obs=jnp.array(1.0),
            pnull=jnp.array(0.05),
        ...
        assert result.expected_bands is bands
