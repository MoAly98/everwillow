...
class TestApplyBoundsTransform:
    def test_returns_unwrapped_state_and_maps(self):
        ...
        state: sl.State[tp.Any] = sl.State.from_pytree({"mu": 0.5})
        transform = transforms.MinuitTransform(lower=0.0, upper=1.0)

        transform_map = {("mu",): transform}
        unwrapped = bounds.unwrap(state, transform_map)  # type: ignore[arg-type]

        assert jnp.isclose(unwrapped["mu",], transform.unwrap(0.5))

        # round-trip using statelib helper
        restored = bounds.wrap(unwrapped, transform_map)  # type: ignore[arg-type]
    ...
    def test_handles_multiple_matches(self):
        ...
        state: sl.State[tp.Any] = sl.State.from_pytree(params)
        transform = transforms.OneSidedLogTransform(bound=0.0, direction="lower")

        transform_map = {("a", "y"): transform, ("b", "y"): transform}
        unwrapped = bounds.unwrap(state, transform_map)  # type: ignore[arg-type]
    ...
    def test_no_transforms_returns_identity(self):
        ...
        state: sl.State[tp.Any] = sl.State.from_pytree({"value": 3.14})
        unwrapped = bounds.unwrap(state, {})  # type: ignore[arg-type]
